...
describe BooksController do
  ...
  context "New Book controller" do
    ...
    it "should have authors auto-complete" do
      ...
      response.should have_tag("script[src=?]", '/autocomplete/authors')
      response.should have_tag("form", /new Autocompleter.Local\('book_authors', 'author_lookup_auto_complete', authors, \{ tokens: ',' \}\);/)
    end
    
    it "should have tags auto-complete" do
      get 'new'
      response.should have_tag("input[type=?][id=?][name=?]", 'text', 'book_tags', 'book[tags]')
      response.should have_tag("script[src=?]", '/autocomplete/tags')
      response.should have_tag("body", /new Autocompleter.Local\('book_tags', 'tags_lookup_auto_complete', tags, \{ tokens: ',' \}\);/)
    end
  end
